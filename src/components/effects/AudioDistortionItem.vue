...
const driveLabel = computed(() => Number(props.effect.params.drive ?? 0).toFixed(2))
const wetLabel = computed(() => Number(props.effect.params.wet ?? 0).toFixed(2))

function handleDriveInput(event) {
  emit('update-param', props.effect.id, 'drive', Number(event.target.value))
}

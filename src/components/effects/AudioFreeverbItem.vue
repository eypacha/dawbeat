      ...
      <div class="flex items-center justify-between gap-3">
        <div class="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-[13px] font-medium leading-tight text-zinc-50">Freeverb</p>
          </div>
        </div>
              ...
              <div class="flex items-center justify-between gap-3 text-[10px] uppercase tracking-[0.24em] text-zinc-500">
                <div class="flex items-center gap-2">
                  <EffectParamAutomationButton
                    :effect-id="effect.id"
                    label="Freeverb Room Size"
                    param-key="roomSize"
                    @create="emit('create-automation', effect.id, 'roomSize')"
                  />
                  <span>Room Size</span>
                </div>
                <span>{{ roomSizeLabel }}</span>
              </div>
              <input
            ...
            <div class="grid gap-2">
              <div class="flex items-center justify-between gap-3 text-[10px] uppercase tracking-[0.24em] text-zinc-500">
                <div class="flex items-center gap-2">
                  <EffectParamAutomationButton
                    :effect-id="effect.id"
                    label="Freeverb Dampening"
                    param-key="dampening"
                    @create="emit('create-automation', effect.id, 'dampening')"
                  />
                  <span>Dampening</span>
                </div>
                <span>{{ dampeningLabel }}</span>
              </div>
              <input
                class="accent-amber-300"
                :value="effect.params.dampening"
                max="10000"
                min="0"
                step="1"
                type="range"
                @blur="emit('interaction-end')"
                @input="handleDampeningInput"
                @keydown="handleInteractionKeydown"
                @keyup="emit('interaction-end')"
                @pointercancel="emit('interaction-end')"
                @pointerdown="emit('interaction-start')"
...
const roomSizeLabel = computed(() => Number(props.effect.params.roomSize ?? 0.7).toFixed(2))
const dampeningLabel = computed(() => `${Math.round(props.effect.params.dampening ?? 3000)} Hz`)
const wetLabel = computed(() => `${Math.round((props.effect.params.wet ?? 0.3) * 100)}%`)

function handleRoomSizeInput(event) {
  emit('update-param', props.effect.id, 'roomSize', Number(event.target.value))
}

function handleDampeningInput(event) {
  emit('update-param', props.effect.id, 'dampening', Number(event.target.value))
}

      ...
      <div class="flex items-center justify-between gap-3">
        <div class="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-[13px] font-medium leading-tight text-zinc-50">Gate</p>
          </div>
        </div>
        ...
        <div v-if="effect.expanded">
          <div class="grid gap-3 pt-4">
            ...
            <div class="grid gap-2">
              <div class="flex items-center justify-between gap-3 text-[10px] uppercase tracking-[0.24em] text-zinc-500">
                <span>Smoothing</span>
                <span>{{ smoothingLabel }}</span>
              </div>
              <input
                class="accent-amber-300"
                :value="effect.params.smoothing"
                max="1"
                min="0"
                step="0.001"
                type="range"
                @blur="emit('interaction-end')"
                @input="handleSmoothingInput"
                @keydown="handleInteractionKeydown"
                @keyup="emit('interaction-end')"
                @pointercancel="emit('interaction-end')"
                @pointerdown="emit('interaction-start')"
...
const thresholdLabel = computed(() => `${Number(props.effect.params.threshold ?? -40).toFixed(1)} dB`)
const smoothingLabel = computed(() => Number(props.effect.params.smoothing ?? 0.1).toFixed(3))

function handleThresholdInput(event) {
  emit('update-param', props.effect.id, 'threshold', Number(event.target.value))
}

function handleSmoothingInput(event) {
  emit('update-param', props.effect.id, 'smoothing', Number(event.target.value))
}

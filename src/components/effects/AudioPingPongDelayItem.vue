      ...
      <div class="flex items-center justify-between gap-3">
        <div class="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-[13px] font-medium leading-tight text-zinc-50">Ping Pong Delay</p>
          </div>
        </div>
...
const delayTimeLabel = computed(() => `${Number(props.effect.params.delayTime ?? 0.25).toFixed(2)} s`)
const feedbackLabel = computed(() => `${Math.round((props.effect.params.feedback ?? 0.5) * 100)}%`)

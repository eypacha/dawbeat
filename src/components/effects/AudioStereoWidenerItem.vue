      ...
      <div class="flex items-center justify-between gap-3">
        <div class="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-[13px] font-medium leading-tight text-zinc-50">Stereo Widener</p>
          </div>
        </div>
              ...
              <div class="flex items-center justify-between gap-3 text-[10px] uppercase tracking-[0.24em] text-zinc-500">
                <div class="flex items-center gap-2">
                  <EffectParamAutomationButton
                    :effect-id="effect.id"
                    label="Stereo Widener Width"
                    param-key="width"
                    @create="emit('create-automation', effect.id, 'width')"
                  />
                  <span>Width</span>
                </div>
...
const widthLabel = computed(() => Number(props.effect.params.width ?? 0).toFixed(2))

function handleWidthInput(event) {
  emit('update-param', props.effect.id, 'width', Number(event.target.value))
}

      ...
      <div class="flex items-center justify-between gap-3">
        <div class="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-[13px] font-medium leading-tight text-zinc-50">Tremolo</p>
          </div>
        </div>
        ...
        <div v-if="effect.expanded">
          <div class="grid gap-3 pt-4">
            <div class="grid gap-2">
              <div class="text-[10px] uppercase tracking-[0.24em] text-zinc-500">Type</div>
              <select
              ...
              >
                <option v-for="t in TREMOLO_TYPES" :key="t" :value="t">{{ t }}</option>
              </select>
            </div>
            ...
            <div class="grid gap-2">
              <div class="flex items-center justify-between gap-3 text-[10px] uppercase tracking-[0.24em] text-zinc-500">
                <div class="flex items-center gap-2">
                  <EffectParamAutomationButton
                    :effect-id="effect.id"
                    label="Tremolo Spread"
                    param-key="spread"
                    @create="emit('create-automation', effect.id, 'spread')"
                  />
                  <span>Spread</span>
                </div>
              ...
              <input
                class="accent-amber-300"
                :value="effect.params.spread"
                max="180"
                min="0"
                step="1"
                type="range"
                @blur="emit('interaction-end')"
                @input="handleSpreadInput"
                @keydown="handleInteractionKeydown"
                @keyup="emit('interaction-end')"
                @pointercancel="emit('interaction-end')"
                @pointerdown="emit('interaction-start')"
...
const TREMOLO_TYPES = ['sine', 'square', 'triangle', 'sawtooth']
...
const frequencyLabel = computed(() => `${Number(props.effect.params.frequency ?? 10).toFixed(1)} Hz`)
const depthLabel = computed(() => `${Math.round((props.effect.params.depth ?? 0.5) * 100)}%`)
const spreadLabel = computed(() => `${Math.round(props.effect.params.spread ?? 180)}°`)
...
function handleSpreadInput(event) {
  emit('update-param', props.effect.id, 'spread', Number(event.target.value))
}

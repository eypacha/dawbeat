      ...
      <div class="flex items-center justify-between gap-3">
        <div class="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-[13px] font-medium leading-tight text-zinc-50">Vibrato</p>
          </div>
        </div>
...
const frequencyLabel = computed(() => `${Number(props.effect.params.frequency ?? 5).toFixed(1)} Hz`)
const depthLabel = computed(() => `${Math.round((props.effect.params.depth ?? 0.1) * 100)}%`)
